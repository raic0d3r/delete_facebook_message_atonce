...
var ssm = setInterval(function() {
    ...
    $("#num").text("Gettting " + $("a[href*='/messages/t/']").map(function(){ var el = $(this).parents('[role="row"]').eq(0); if(el.css("visibility") != "hidden") return el[0];}).length + " Messages. Please wait.");
    // console.log(tt);
    if (tt == 5) {
        deletefbmessage();
        clearInterval(ssm);
...
function deletefbmessage() {
      ...
      var elms = $("a[href*='/messages/t/']").map(function(){
        var el = $(this).parents('[role="row"]').eq(0);
        if(el.css("visibility") != "hidden") return el[0];
      }).eq(0);

      var button = elms.find('.x10f5nwc.xi81zsa');
